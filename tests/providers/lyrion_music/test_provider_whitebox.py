...
async def test_handle_async_init_calls_serverstatus(
    ...
    del lyrion_provider
    assert any(call.command == "serverstatus" for call in fake_lms_server.rpc_calls)
...
async def test_library_iterators_issue_expected_entity_rpcs(
    ...
    assert any(call.command == "artists" for call in fake_lms_server.rpc_calls)
    assert any(call.command == "albums" for call in fake_lms_server.rpc_calls)
    assert any(call.command == "titles" for call in fake_lms_server.rpc_calls)
...
async def test_resolve_image_falls_back_to_300px(
    ...
    assert any(
        path.endswith("/contributor/a3/image_600x600_f") for path in fake_lms_server.image_requests
    )
    assert any(
        path.endswith("/contributor/a3/image_300x300_f") for path in fake_lms_server.image_requests
    )
...
async def test_track_lookup_falls_back_from_broken_batch_responses(
    ...
    fake_lms_server.clear_history()
    fake_lms_server.incomplete_batch_for_entities.add("titles")
    ...
    track_id_filters = [
        call.args[4]
        for call in fake_lms_server.rpc_calls
...
async def test_rpc_error_is_translated_to_provider_unavailable(
    ...
    fake_lms_server.command_errors["artists"] = (-32001, "server exploded politely")
